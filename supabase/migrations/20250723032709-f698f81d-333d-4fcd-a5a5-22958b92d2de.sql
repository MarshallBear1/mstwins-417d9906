-- Fix the configuration settings that are missing
-- First, we need to set these properly in the database
ALTER DATABASE postgres SET app.settings.supabase_url = 'https://fscendubnktdtmnxiipk.supabase.co';

-- Since we can't directly set the service role key in migration, let's create a simpler approach
-- We'll modify the triggers to use a different method that doesn't rely on configuration
